...
function getActionVariant(actionType: string): "supply" | "withdraw" | "borrow" | "repay" | "claim" | "supply_collateral" | "withdraw_collateral" | "secondary" {
  const variants: Record<string, "supply" | "withdraw" | "borrow" | "repay" | "claim" | "supply_collateral" | "withdraw_collateral"> = {
    'supply': 'supply',
    'withdraw': 'withdraw',
...
export function ActionsBackfill() {
  ...
  async function uploadCsv() {
    ...
    try {
      ...
      const response = await fetch('/api/bigquery/actions/upload-csv', {
        method: 'POST',
        body: formData,
      })
      const data = await response.json()
  ...
  return (
    <Card className="border-border/50">
      <CardHeader>
        <CardTitle className="text-2xl">Blend Actions Backfill</CardTitle>
        <CardDescription>
          Backfill action events (supply, withdraw, borrow, repay, claim) from BigQuery
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg">
          <div>
            <p className="text-xs text-muted-foreground mb-1">Tracked Pools</p>
            <div className="text-sm space-y-0.5">
              {config?.tracked_pools?.map((pool, i) => (
                <div key={i} className="font-mono text-xs">{truncateAddress(pool)}</div>
                    ...
                    <TableBody>
                      {simulateResult.rows.map((row, i) => (
                        <TableRow key={i}>
                          ...
                          <TableCell className="font-mono text-xs">
                            {truncateAddress(row.asset_address)}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatNumber(row.amount_underlying)}
                          </TableCell>
